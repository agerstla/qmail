...
This is an updated and heavily patched `netqmail` package for Debian systems to replace the one that got removed from newer distributions.
It is forked from the `no-chkuser-vpopmail` branch of the `qmail` repository [maintained by Roberto Puzzanghera](https://github.com/sagredo-dev/qmail) to use [`qmail-verify` from Andrew Richards](https://free.acrconsulting.co.uk/email/qmail-verify.html) for local user verification (and to support Debian-specific packaging).
